...
WNT2_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.82 Safari/537.36'
...
def actionEpisodesMenu(params):
    ...
    if lastListURL and lastListURL == params['url']:
        ...
        html = requestHelper(url if url.startswith('http') else BASEURL + url)
...
def actionLatestMoviesMenu(params):
    ...
    if not html:
        html = requestHelper(BASEURL + params['path'])
...
def actionGenresMenu(params):
    html = requestHelper(BASEURL + URL_PATHS['genre'])
...
def getPageMetadata(html):
    ...
    if stringStartIndex != -1:
        ...
        if '/anime/movies' not in parentURL:
            html = requestHelper(parentURL if parentURL.startswith('http') else BASEURL + parentURL)
...
def makeLatestCatalog(params):
    ...
    html = requestHelper(BASEURL + '/last-50-recent-release')
...
def makePopularCatalog(params):
    html = requestHelper(BASEURL) # We will scrape from the sidebar content on the homepage.
...
def makeSeriesSearchCatalog(params):
    ...
    html = requestHelper( BASEURL+'/search', data={'catara': params['query'], 'konuara': 'series'}, extraHeaders={'Referer': BASEURL+'/'} )
...
def makeMoviesSearchCatalog(params):
    ...
    html = requestHelper(BASEURL + URL_PATHS['movies'])
...
def actionResolve(params):
    ...
    content = requestHelper(url.replace('watchcartoononline.io', 'wcofun.com', 1)) # New domain safety replace.
    ...
    html = requestHelper(unescapeHTMLText(embedURL)) # Sometimes a '&#038;' symbol is present in this URL.
...
def requestHelper(url, data=None, extraHeaders=None, output=''):
    ...
    response = client.request(url, post=data, headers=myHeaders, verify=False, output=output, timeout=10)
